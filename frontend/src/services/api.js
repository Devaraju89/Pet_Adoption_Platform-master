...
const API_BASE_URL = (import.meta.env.VITE_API_BASE_URL || "https://pet-adoption-platform-master-1.onrender.com") + "/api";
...
export const adoptionAPI = {
  createRequest: (data) => api.post("/adoption-requests", data),
  getMyRequests: () => api.get("/adoption-requests/my-requests"),
  getAllRequests: () => api.get("/adoption-requests"), // Endpoint for admin to fetch all requests
  updateStatus: (id, statusData) => api.patch(`/adoption-requests/${id}/status`, statusData),
};

// User API (Favorites)
export const userAPI = {
  toggleFavorite: (petId) => api.post(`/user/favorites/${petId}`),
