...
export default function PetDetails() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const [petRes, favRes] = await Promise.all([
          petsAPI.getPetById(id),
          user ? userAPI.getFavorites() : Promise.resolve({ data: [] })
        ]);
          ...
          <div className="lg:flex">
            ...
            <div className="lg:w-1/2 relative group">
              ...
              <div className="absolute top-4 right-4 z-20">
                <span className={`px - 3 py - 1.5 rounded - full text - xs font - bold shadow - lg ${pet.adoptionStatus === 'available' ? 'bg-green-500 text-white' :
            ...
            <div className="lg:w-1/2 p-6 lg:p-8 flex flex-col">
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <span className={`px - 3 py - 1.5 rounded - md text - xs font - semibold border ${pet.isVaccinated ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-50 text-gray-600 border-gray-200'
                    } `}>
                    {pet.isVaccinated ? '✓ Vaccinated' : '○ Not Vaccinated'}
                  </span>
                  <span className={`px - 3 py - 1.5 rounded - md text - xs font - semibold border ${pet.isNeutered ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-gray-50 text-gray-600 border-gray-200'
                    } `}>
              ...
              {feedback && (
                <div className={`mt - 3 p - 2.5 rounded - lg font - semibold text - xs ${feedback.type === "error" ? "bg-red-100 text-red-700 border border-red-300" : "bg-green-100 text-green-700 border border-green-300"
                  } `}>
                  {feedback.text}
                </div>
              )}
