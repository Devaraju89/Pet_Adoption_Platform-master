...
export default function Login() {
  ...
  const handleLogin = async () => {
    try {
      ...
      // Navigate to welcome page and reload once to update app state
      navigate("/welcome");
  ...
  const handleGoogleSignIn = () => {
    ...
    const popup = window.open(
      `${import.meta.env.VITE_API_URL}/auth/google?prompt=select_account`,
      "googleAuth",
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-pink-100 via-purple-50 to-rose-100 relative overflow-hidden">
          ...
          <div className="text-center animate-bounce-gentle">
            ...
            <div className="relative mb-8">
              <div className="w-48 h-48 bg-white rounded-full shadow-2xl relative mx-auto">
                ...
                {/* Inner Ears */}
                <div className="absolute -top-6 left-14 w-12 h-12 bg-pink-200 rounded-full"></div>
                <div className="absolute -top-6 right-14 w-12 h-12 bg-pink-200 rounded-full"></div>
              ...
              {/* Floating Hearts */}
              <div className="absolute -top-4 -right-4 text-pink-400 text-2xl animate-float-up">💕</div>
              <div className="absolute -bottom-2 -left-6 text-rose-400 text-lg animate-float-up" style={{ animationDelay: '1s' }}>💖</div>
            </div>

            <h2 className="text-3xl font-bold text-gray-800 mb-2">Welcome Back!</h2>
            <p className="text-gray-600 text-lg">Sign in to continue your pet journey</p>
          </div>
        </div>
      </div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-end">
              <Link to="/forgot-password" className="text-sm text-pink-600 hover:text-pink-700 font-medium transition-colors duration-200">
                Forgot Password?
              </Link>
            </div>
            ...
            <button
              onClick={handleLogin}
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-pink-500 to-purple-600 text-white py-3 px-4 rounded-xl font-semibold text-lg shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
            >
              {isLoading ? (
